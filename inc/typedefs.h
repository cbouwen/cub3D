...
//struct to store all relevant player data for raycasting. Position vector,
	direction vector, camera plane vector
...
typedef struct	s_raycaster
{
	...
	// double time
		&& double oldTime? ==> Do we want to implement FPS? I fucking think not
}					t_raycaster;

//Combine this data in a massive struct, which can be far too difficult to use,
	let alone init to default
//Add all the tiny structs to the t_mlx_data struct as a reference,
	keeps all the struct seperate but allow them to be used everywhere else without passing more than one parameter

//Additionally,
	we should add a seperate variable in one of the map structs to store the actual map,
